...
CSV.write(
    "$(Dates.format(now(),"mmddyy"))_hist_ATP_AUC_$(n_bootstrap)runs_$(fold_range)x_range_only_supported_ATP_supply.csv",
...
CSV.write(
    "$(Dates.format(now(),"mmddyy"))_ATP_AUC_gsa_sobol_$(n_samples)_$(fold_range)x_range.csv",
    df,
)
